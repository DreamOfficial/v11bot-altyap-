...
exports.run = async(client, message, args) => {
  ...
  if(!message.member.hasPermission('SEND_MESSAGES')) return message.reply('**Üzgünüm Bu Komutu Kullanabilmek İçin Yeterli İzine Sahip Değilsin!**')
  ...
  if(panel) return message.channel.send(`Bu sunucuda panel zaten ayarlanmış! Devredışı bırakmak için;  \`${prefix}sunucupanel sil\``)
      ...
      message.channel.send(new Discord.RichEmbed().setColor('RANDOM').setTitle('Sunucu Panel').setDescription('Gerekli Dosaylar Kurulsun Mu?.').setFooter('Onaylıyorsan 15 Saniye İçerisinde "evet" Yazmalısın.'))
.then(() => {
